...
# 창 위치 설정 (Windows의 경우)
os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % (800, 100)  # 화면의 오른쪽 구석으로 창 위치 설정
...
DISPLAY_WIDTH = 800
DISPLAY_HEIGHT = 600
main_display = pygame.display.set_mode((DISPLAY_WIDTH, DISPLAY_HEIGHT), pygame.HWSURFACE | pygame.DOUBLEBUF)
pygame.display.set_caption('Main Display')
...
# 웨이포인트 시각화 함수
def draw_waypoints(display, map, vehicle_location, scale=1.5, point_radius=3, close_point_radius=5, radius=10):
    """
    전체 웨이포인트와 차량 주변의 웨이포인트를 시각화하는 함수
    """
    display.fill((0, 0, 0))  # 배경 초기화
    # 전체 웨이포인트 시각화
...
def main():
    ...
    try:
        while True:
            ...
            draw_waypoints(main_display, world.get_map(), vehicle_location)
