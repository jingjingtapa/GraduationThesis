...
scene_delay = 10
condition = 'morning_foggy'
num_img, num_scene = 30, 25
...
map_x_left, map_y_left, bev_left_height, bev_left_width = bev.generate_direct_backward_mapping(math.trunc(width/2), wy_max, wy_interval, -10, 10, wx_interval, R_left, K)
map_x_right, map_y_right, _, _ = bev.generate_direct_backward_mapping(math.trunc(width/2), wy_max, wy_interval, -10, 10, wx_interval, R_left, K)
...
if not os.path.isdir(f'{dir_download}/{condition}'):
    os.makedirs(f'{dir_download}/{condition}')

if not os.path.isdir(f'{dir_download}/{condition}/images'):
    os.makedirs(f'{dir_download}/{condition}/images')

if not os.path.isdir(f'{dir_download}/{condition}/labels'):
    os.makedirs(f'{dir_download}/{condition}/labels')

def save(screen, traffic_list, cnt_scene, cnt_img):
    dir_scene_img = f'{dir_download}/{condition}/images/{cnt_scene}'
    dir_scene_lbl = f'{dir_download}/{condition}/labels/{cnt_scene}'
...
cnt_scene, cnt_img = 1, 1
while running and cnt_scene <= num_scene:
    world.tick()
            ...
            if distance <= wx_max:
                ...
                for coord in bbox_ego_coords:
                    ...
                    if 0 <= bev_x < screen_width and 0 <= bev_y < screen_height:  # BEV 이미지 내에 있는지 확인
                        vehicle_bev_coord.append(round(bev_x/bev_height,3))
                        vehicle_bev_coord.append(round(bev_y/bev_width,3))
    ...
    if cnt_img <= num_img:
        ...
        cnt_img += 1
    elif num_img < cnt_img <= num_img*5:
        cnt_img += 1
    elif cnt_img > num_img*5:
        cnt_img = 1
        cnt_scene += 1
